...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#4a0404] text-white font-sans pb-20">
      ...
      {/* Header */}
      <header className="sticky top-0 z-50 bg-red-950/90 backdrop-blur-md border-b-2 border-yellow-600/20 px-4 sm:px-6 py-2 sm:py-4 flex items-center justify-between">
        <div className="flex items-center gap-2 sm:gap-3 cursor-pointer" onClick={() => { setActiveGame(null); setShowLeaderboard(false); }}>
          <span className="text-xl sm:text-3xl">🌞</span>
          <h1 className="text-lg sm:text-2xl font-black text-yellow-500 uppercase italic tracking-tighter leading-none">Avurudu Hub</h1>
        </div>
      ...
      <main className="max-w-4xl mx-auto p-6">
        <AnimatePresence mode="wait">
          {!activeGame && !showLeaderboard && (
            ...
            >
              {/* Hero Section */}
              <section className="bg-gradient-to-br from-red-800 to-red-950 p-5 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border-4 border-yellow-600/30 relative overflow-hidden">
                <div className="relative z-10">
                  <h2 className="text-xl sm:text-4xl font-black text-yellow-500 mb-2 uppercase italic leading-tight text-balance">Subha Aluth Avuruddak Wewa!</h2>
                  <p className="text-yellow-200/70 mb-5 max-w-md text-xs sm:text-base leading-relaxed">Celebrate the Sinhala & Tamil New Year with our digital traditional games. Compete globally!</p>
                  
                  <div className="mb-6 max-w-sm scale-95 origin-left sm:scale-100">
                    <p className="text-[9px] font-black text-yellow-600 uppercase tracking-[0.3em] mb-2">Countdown to Avurudu</p>
                    <Countdown />
                  </div>
                  ...
                  <div className="flex flex-row gap-2 sm:gap-4">
                    <button 
                      ...
                      className="flex-1 sm:flex-none bg-yellow-500 text-red-900 font-black px-4 sm:px-6 py-3 rounded-xl flex items-center justify-center gap-2 hover:bg-yellow-400 transition-colors text-[11px] sm:text-base uppercase"
                    >
                      <Trophy size={16} /> <span className="hidden xs:inline">Leaderboard</span><span className="xs:hidden">Scores</span>
                    </button>
                    <button 
                      onClick={() => setShowAdmin(true)}
                      className="flex-1 sm:flex-none bg-red-900/40 border-2 border-yellow-600/30 text-yellow-500 font-black px-4 sm:px-6 py-3 rounded-xl flex items-center justify-center gap-2 hover:bg-red-900/60 transition-colors text-[11px] sm:text-base uppercase"
                    >
                      <Settings size={16} /> Admin
                    </button>
                  </div>
                </div>
                <div className="absolute -right-6 -bottom-6 text-[6rem] sm:text-[12rem] opacity-10 rotate-12">🌞</div>
              </section>
          ...
          {activeGame && (
            ...
            >
              <button 
                ...
                className="text-yellow-500 font-black uppercase tracking-widest flex items-center gap-2 hover:translate-x-[-4px] transition-transform text-xs sm:text-sm"
              >
                ← Back to Hub
              </button>
