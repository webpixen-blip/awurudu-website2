...
export const GreasyPole: React.FC<GreasyPoleProps> = ({ username, soundEnabled, onComplete }) => {
  ...
  return (
    <div className="bg-red-900/40 backdrop-blur-md rounded-3xl border-4 border-yellow-600/30 p-4 sm:p-8 shadow-2xl overflow-hidden relative h-[550px] sm:h-[650px] flex flex-col">
      <div className="text-center mb-4 sm:mb-6">
        <h2 className="text-2xl sm:text-3xl font-black text-yellow-500 uppercase italic tracking-tighter">Grees Gahe Nagima</h2>
        <p className="text-yellow-200/60 text-xs sm:text-sm">Tap rapidly to climb the greasy pole!</p>
      </div>

      <div className="flex-1 flex gap-4 sm:gap-8 min-h-0">
        {/* The Pole Area */}
        <div className="relative w-20 sm:w-32 bg-red-950/50 rounded-full border-2 border-yellow-600/20 flex flex-col items-center py-8 overflow-hidden">
          {/* The Actual Pole */}
          <div className="absolute inset-y-0 w-3 sm:w-6 bg-gradient-to-r from-yellow-900 via-yellow-600 to-yellow-900 rounded-full shadow-inner" />
          
          {/* The Grease Effect */}
          <div className="absolute inset-0 opacity-20 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-black via-transparent to-transparent pointer-events-none" />
          ...
          <div className="absolute top-4 z-10">
            ...
            >
              <Flag className="text-red-900" size={20} />
            </motion.div>
          </div>
            ...
            <div className={`relative ${isSliding ? 'animate-bounce' : ''}`}>
              ...
              {gameState === 'playing' && (
                <motion.div 
                  ...
                  className="absolute -top-4 -right-4 bg-yellow-500 text-red-900 text-[8px] sm:text-[10px] font-black px-1.5 sm:px-2 py-0.5 rounded-full"
                >
                  {Math.round(height)}%
                </motion.div>
              )}
        ...
        <div className="flex-1 flex flex-col justify-center gap-4 sm:gap-6">
          <div className="bg-red-950/50 p-3 sm:p-4 rounded-2xl border border-yellow-600/20">
          ...
          {gameState === 'playing' && (
            <button
              onMouseDown={handleClimb}
              onTouchStart={(e) => { e.preventDefault(); handleClimb(); }}
              className="w-full aspect-square max-w-[150px] sm:max-w-none mx-auto bg-yellow-500 text-red-900 font-black rounded-full flex flex-col items-center justify-center gap-1 sm:gap-2 shadow-[0_8px_0_rgb(180,130,0)] sm:shadow-[0_12px_0_rgb(180,130,0)] active:translate-y-2 active:shadow-[0_4px_0_rgb(180,130,0)] transition-all select-none touch-none"
            >
              <ArrowUp size={32} className="sm:w-12 sm:h-12" />
              <span className="text-base sm:text-xl uppercase italic">Climb!</span>
            </button>
          )}
