...
export const AdminPortal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full max-w-5xl bg-red-950 border-4 border-yellow-600/30 p-5 sm:p-8 rounded-[1.5rem] sm:rounded-[2.5rem] my-auto min-h-[80vh]">
        ...
        {/* Tabs */}
        <div className="flex gap-2 mb-6 sm:mb-8 bg-red-900/20 p-2 rounded-xl sm:rounded-2xl overflow-x-auto no-scrollbar scroll-smooth">
          {(['nakath', 'quiz', 'raban'] as AdminTab[]).map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`flex-1 min-w-[100px] py-3 rounded-lg sm:rounded-xl font-black uppercase text-[11px] sm:text-sm tracking-widest transition-all flex items-center justify-center gap-2 ${
                activeTab === tab ? 'bg-yellow-500 text-red-950 shadow-lg' : 'text-yellow-600 hover:bg-white/5'
              }`}
            >
